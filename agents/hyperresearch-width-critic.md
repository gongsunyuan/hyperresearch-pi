...
tier: opus
description: Use this agent in Layer 5 of the hyperresearch deep research pipeline. Reads the Layer 4 draft and returns a findings list of topics the width corpus supports but the draft doesn't cover. Runs on Opus. Spawn ONCE per draft, parallel with dialectic-critic and depth-critic.
...
1. **Survey the vault.** Use
   `hyperresearch search "" --tag <vault_tag> -j` to list every note.
