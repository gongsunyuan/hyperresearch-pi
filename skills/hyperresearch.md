...
<!-- Ported from hyperresearch (Claude Code) to pi. Skill() calls map to the load_skill tool; hyperresearch CLI calls map to hr_* tools. -->

## ⚙ pi 适配说明（必读）

本 skill 从 Claude Code 移植到 pi。关键差异：

1. **加载步骤 skill**：原版用 `Skill(skill: "hyperresearch-N-xxx")`，pi 中改为调用 **`load_skill`** 工具：
   ```
   load_skill(name: "hyperresearch-N-xxx")
   ```
   该工具返回对应步骤 skill 的完整指令，你在当前上下文执行后返回本文件加载下一步。

2. **vault 操作**：原版通过 bash 执行 `hyperresearch <cmd> --json`，pi 中改为调用对应的 **`hr_*`** 工具（它们内部调用 hyperresearch CLI 并解析 JSON）：
   | 原 CLI | pi 工具 |
   |---|---|
   | `hyperresearch search` | `hr_search` |
   | `hyperresearch fetch` | `hr_fetch` |
   | `hyperresearch fetch-batch` | `hr_fetch_batch` |
   | `hyperresearch note show` | `hr_note_show` |
   | `hyperresearch note new` | `hr_note_new` |
   | `hyperresearch note update` | `hr_note_update` |
   | `hyperresearch graph` | `hr_graph` |
   | `hyperresearch lint` | `hr_lint` |
   | `hyperresearch status` | `hr_status` |
   | `hyperresearch sync` | `hr_sync` |
   | `hyperresearch vault-tag` | `hr_vault_tag` |
   | `hyperresearch archive-run` | `hr_archive_run` |
   | `hyperresearch tags` | `hr_tags` |
...
3. **子代理（subagent）**：原版用 Claude Code 的 `Task` 工具 spawn `.claude/agents/*.md`，pi 中这些 agent 已定义为 pi subagent（见包内 agents/ 目录，session_start 时自动部署到 `.pi/agents/hyperresearch/`）。用 **`subagent`** 工具的 SINGLE 或 PARALLEL 模式 spawn，agent 名不变（如 `hyperresearch-fetcher`、`hyperresearch-loci-analyst`）。

   **模型分层（机器级配置文件）**：agent 源文件只标 `tier: opus|sonnet`，不含供应商信息。扩展启动时读取 `~/.pi/agent/hyperresearch-models.json`，按 tier 注入 `model` + `fallbackModels` 到部署后的 agent frontmatter。换 provider 只改这一个 JSON 文件，重启 / `/reload` 生效。

   配置文件结构（JSON）：
   ```json
   {
     "tiers": {
       "opus": { "model": "shiyun/claude-opus-4-8", "fallbackModels": ["shiyun/claude-sonnet-5"] },
       "sonnet": { "model": "shiyun/claude-sonnet-5", "fallbackModels": ["zai-glm/glm-5.2"] }
     },
     "agents": {
       "hyperresearch-patcher": { "model": "openai/gpt-4o", "fallbackModels": ["zai-glm/glm-5.2"] }
     }
   }
   ```

   pi-subagents 会在主模型因无 API key / 限流等可重试错误失败时自动依次尝试 fallbackModels。整条链都不可用则 spawn 硬失败。工具锁（patcher/polish 仅 Read+Edit）已在 agent 配置中保留。

4. **TodoWrite** → pi 内置 `todo` 工具，用法相同。

5. **bash 工具**：仍有 `bash` 工具可用于文件读写（如写 `research/scaffold.md`）。但凡涉及 vault 数据的操作一律走 hr_* 工具。
...
3. Invoke each step skill in sequence via the `load_skill` tool.
...
```
load_skill(name: "hyperresearch-N-stepname")
```
...
0. **Auto-init if missing.** Two checks for the first-run case:
   - **Vault check.** If `.hyperresearch/` doesn't exist in the working directory, call the `hr_init` tool (with an optional vault name). Creates the SQLite vault and `research/` directory.
   - **Step-skills check.** In pi, the 16 step skills are bundled in the hyperresearch-pi package and loaded on-demand by the `load_skill` tool — no install step needed. Skip the `hyperresearch install --steps-only` command from the Claude Code version; it is unnecessary here.

   If `hr_init` fails because the hyperresearch Python CLI is not installed, tell the user to run `pip install hyperresearch` first.

0.5. **Archive any prior run's artifacts.** Call the `hr_archive_run` tool. If a previous `/hyperresearch` session left a scaffold, loci.json, comparisons.md, critic-findings, patch-log, polish-log, prompt-decomposition, or any `research/temp/*` scratch, this moves the whole set into `research/runs/archive-<prev-tag>-<UTC-timestamp>/` so the new run starts from a clean slate without losing the prior run's audit trail. Final reports (`research/notes/final_report_<tag>.md`) and canonical query files (`research/query-<tag>.md`) are already namespaced and stay in place. The tool no-ops cheaply on a fresh vault — safe to run unconditionally.
...
2. **Mint a unique vault tag.** First produce a short topical slug from the canonical query — 3–5 lowercase hyphen-separated words, e.g. `efield-dft-sac`. Then call the `hr_vault_tag` tool with `slug` set to that slug and parse the `vault_tag` field from the response. The CLI appends a random 6-hex-char suffix that's verified unique against every prior run's `research/query-*.md` and `research/notes/final_report_*.md` in this vault. The result — e.g. `efield-dft-sac-a3f9b7` — is the canonical vault_tag for the rest of the pipeline. The suffix guarantees no overwrite of a prior run's final report or query file, even if the user re-runs the exact same query or two different queries slug-collide.
...
7. **Invoke step 1:** `load_skill(name: "hyperresearch-1-decompose")`.
...
4. **Re-invoke this entry skill** if you've lost track entirely: `load_skill(name: "hyperresearch")`. It loads fresh.
...
```
load_skill(name: "hyperresearch-1-decompose")
```
