...
5. **Exit and route.** Once `research/notes/final_report_<vault_tag>.md` is written, return to the entry skill and invoke `load_skill(name: "hyperresearch-15-polish")`. Light tier skips steps 11–14 entirely.
...
- **light tier:** You already wrote `research/notes/final_report_<vault_tag>.md` directly. Skip steps 11-14 (no synthesis, no critics, no patcher) and invoke `load_skill(name: "hyperresearch-15-polish")`.
- **full tier:** Invoke `load_skill(name: "hyperresearch-11-synthesize")`.
